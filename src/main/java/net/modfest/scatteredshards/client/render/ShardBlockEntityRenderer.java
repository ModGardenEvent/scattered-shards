...
public class ShardBlockEntityRenderer implements BlockEntityRenderer<ShardBlockEntity> {
	...
	public void render(ShardBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, Vec3d cameraPos) {
		...
		shard.icon().ifLeft(stack -> {
			matrices.translate(((shardType.getTextureSettings().getSize().width() - 16) * metersPerPixel / 2) - (offset.left() * metersPerPixel), ((shardType.getTextureSettings().getSize().height() - 16) * metersPerPixel / 2) - (offset.up() * metersPerPixel), -0.002f); //extra -0.002 here to prevent full-cubes from zfighting the card
